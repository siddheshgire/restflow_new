  ...
  useEffect(() => {
    ...
    if (newAssignments.length > 0) {
      ...
      if (newOrder) {
        ...
        playTingSound();
        sendNativePush("New Table Assignment!", `You have been assigned Table ${newOrder.tableId}`);
      }
    }
    ...
    readyIds.forEach(id => {
      if (!prevReadyOrderIds.current.has(id)) {
        ...
        if (order) {
          ...
          playTingSound();
          sendNativePush("Order Ready!", `Order for Table ${order.tableId} is ready to serve!`);
        }
      }
    });
    prevReadyOrderIds.current = readyIds;
