  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      if (!isInitializedRef.current) {
        ...
        if (hasNewPending) {
          playKitchenBell();
          const newOrder = fetched.find(o => o.status === 'pending' && !prevOrderIdsRef.current.has(o.id));
          if (newOrder) {
            sendNativePush("New Order Received!", `Table ${newOrder.tableId} placed a new order.`);
          }
        }
        prevOrderIdsRef.current = currentIds;
      }
